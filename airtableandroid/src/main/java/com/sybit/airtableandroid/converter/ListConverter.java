...
public class ListConverter extends AbstractConverter {
   ...
   *
   * This Method handles the conversion from a List of LinkedHashMaps to a List of Attachement
   * Objects.
   *
   * <p>If the Input Object is no List or the List Item is no LinkedHashMap everything will be
   * converted into a (List?) String.</p>
   *
   * @param type The type of the Input Object
   * @param value The value of the Input Object
   * @return A List
   * @throws InstantiationException If no Instance of the listClass can be instantiated
   * @throws IllegalAccessException If the Object can't be accest
   * @throws InvocationTargetException If th Target can't be accessed
   */
  @Override
  protected <T> T convertToType(final Class<T> type, Object value)
      ...
      for (T item : ((List<T>) value)) {
        if (item instanceof LinkedTreeMap) {
          ...
          for (String key : ((LinkedTreeMap<String, Object>) item).keySet()) {
            ...
            BeanUtil.pojo.setProperty(instanz, key, val);
